...
if (!$getOptionsOk) {
    print "Failed to parse options (see above), aborting.\n";
    exit(1);
}
...
if ($orphans) {
    ...
    print "If you want to handle them, you may still abort now.\n";
    print "Type 'YES' to proceed regardless.\n";
    my $ans = <>;
    chomp $ans;
    if ($ans ne 'YES') {
...
if (open(my $fh, '<', '/proc/mounts')) {
    while (<$fh>) {
        ...
        next if ($fields[1] =~ m{^/(dev|sys|proc|snap|boot)(/|$)});
...
if (!@cmds) {
    print "There is nothing to change, all UIDs/GIDs are in sync :)\n";
    exit 0;
}
else {
    print "\n";
...
EOF
    ...
    print "Note that you'll have to reboot once the script has completed.\n";
}
else {
...
sub handle_list {
    ...
    foreach my $master_id (sort keys %{$master->{'name_by_id'}}) {
        ...
        if (exists $local->{'name_by_id'}{$master_id}) {
            ...
            if ($local_name eq $master_name) {
                ...
                print sprintf(
                    "Name collision on $idtype: master $idtype %d exists on local but with a different name (master=%s local=%s)\n",
                ...
                if ($pass == 1) {
                    ...
                    print "-> okay, offsetting local UID $master_id to $new_local_id\n";
                    return 0;
                }
                else {
            ...
            if (defined $local_id) {
                ...
                # don't report if $local_id is > $offset in pass 1, because it's a transient situation WE created, and we know about it
                print sprintf(
                    "Differing name attached to same $idtype: master $idtype %s doesn't exist on local, but its corresponding name '%s' does, with local $idtype %d\n",
                    $master_id, $master_name, $local_id)
                  if ($pass == 2 || $local_id < $offset);
                if ($pass == 1) {
                    ;    # do nothing on first pass, just report
                }
                if ($pass == 2) {
                    ...
                    print "-> okay, setting local $idtype of $master_name to $master_id instead of $local_id\n";
                    # and we ask our caller to call us again as we've changed the local ID for this account or group
                    return 0;
                }
            }
    ...
    if ($pass == 0) {
        ...
        foreach my $local_id (sort keys %unseen_ids) {
            ...
            print sprintf(
                "WARN: local orphan $typename: local $typename %d (with name '%s') is only present locally, if you want to keep it, create it on the master first or it'll be erased\n",
                $local_id, $local_name);
            $orphans++;
        }
        ...
        foreach my $local_name (sort keys %unseen_names) {
            ...
            next if $local_id >= $offset;
            print sprintf(
                "WARN: local orphan name: local name '%s' (with $idtype %d) is only present locally, if you want to keep it, create it on the master first or it'll be erased\n",
                $local_name, $local_id);
            $orphans++;
        }
        # print "OK: $typename: $fullsync fully matching entries, and $tocreate to-be-created entries without collision\n";
    }
